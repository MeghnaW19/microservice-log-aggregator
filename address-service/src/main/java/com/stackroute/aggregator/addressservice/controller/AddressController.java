...
public class AddressController {
    ...
    public ResponseEntity<Address> getAddress(@PathVariable(name = "customerId", required = true) int customerId){
        ...
        Optional<Address> result = addressRespository.findById(customerId);
        return result.map(address -> new ResponseEntity<>(address, HttpStatus.ACCEPTED)).orElseGet(() -> new ResponseEntity<>(HttpStatus.NO_CONTENT));
    }
}
